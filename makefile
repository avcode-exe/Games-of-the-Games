CXX = g++
CPPFLAGS = -pedantic-errors -Werror -std=c++17 -fPIE -I. -I./Maze -I./Player -I./Minesweeper
...
# Header files
HEADERS = $(wildcard $(SRC_DIR)/*.h) $(wildcard $(SRC_DIR)/Maze/*.h) $(wildcard $(SRC_DIR)/Player/*.h) $(wildcard $(SRC_DIR)/Minesweeper/*.h)
...
# Source files
SRCS = $(wildcard $(SRC_DIR)/*.cpp) $(wildcard $(SRC_DIR)/Maze/*.cpp) $(wildcard $(SRC_DIR)/Player/*.cpp) $(wildcard $(SRC_DIR)/Minesweeper/*.cpp)
...
# Link the executable
$(TARGET): $(OBJS) $(MAZE_SO) $(PLAYER_SO) $(MINESWEEPER_SO) | $(BIN_DIR)
	$(CXX) $(CPPFLAGS) -o $@ $^ $(LDFLAGS) -L$(LIB_DIR) -lmaze -lplayer -lminesweeper
...
$(BIN_DIR) $(OBJ_DIR) $(OBJ_DIR)/Maze $(BIN_DIR)/Maze $(OBJ_DIR)/Player $(BIN_DIR)/Player $(OBJ_DIR)/Minesweeper $(BIN_DIR)/Minesweeper $(LIB_DIR):
	mkdir -p $@
