...
void minesweeper_main() {
	...
	while (true) {
		...
		if (user_input == 'q') {
			...
			game.player_y = (game.player_y > 0) ? game.player_y - 1 : game.player_y;
		}
		else if (user_input == KEY_DOWN) {
			game.player_y = (game.player_y < game.rows - 1) ? game.player_y + 1 : game.player_y;
		}
		else if (user_input == KEY_LEFT) {
			game.player_x = (game.player_x > 0) ? game.player_x - 1 : game.player_x;
		}
		else if (user_input == KEY_RIGHT) {
			game.player_x = (game.player_x < game.cols - 1) ? game.player_x + 1 : game.player_x;
		}
		refresh();
